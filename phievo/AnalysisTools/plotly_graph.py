...
def plot_pareto_fronts(self,generations,max_rank=1,with_indexes=False,legend=False,xlim=[],ylim=[],colors=[],gradient=[],xlabel="F_1",ylabel="F_2",s=50,no_popup=False,shapes=[]):
    """
    ...
    if len(shapes) == 0:shapes = ["circle","square","triangle-up"]
    else: shapes = shapes
    data = []
    for gen,ranks in sorted(fitnesses.items(), key=lambda x:x[1]):
        for ind,rank in enumerate(ranks):
            ...
            if len(rank[0]) == 3:
                F1,F2,F3 = zip(*rank)
                trace =  go.Scatter(x = F2,y = F3,mode = 'markers',name="G{0}-rank {1}".format(gen,ind),
                                marker= dict(size=14,line=dict(width=1,color='DarkSlateGrey'),color= colors[gen],colorscale="inferno", colorbar=dict(thickness=20),symbol=shapes[ind]),
                                hoverinfo="text",
...
def Plot_TimeCourseQty(self,trial_index,cell=0,no_popup=False):
    """
    Searches in the data last stored in the Simulation buffer for the time course
    corresponding to the trial_index and the cell and plot the gene time series
    ...
    for i in range(nSpecies):
        if i == 0:
            ...
            trace = go.Scatter(
                x=time_axis,
                y=np.multiply(time_course[:,i],time_course[:,0]),
                name=species_labels[i],
                line = dict(color=colors[i],width=linewidth)
            )
        data.append(trace)
    ...
    if not no_popup:
        plotfunc(fig)
    return fig
setattr(Simulation,"Plot_TimeCourseQty",Plot_TimeCourseQty)
