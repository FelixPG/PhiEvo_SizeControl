...
/**************Transcription rates*****************/
	 ...
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(0.369408*HillA(history[1][memory][ncell],0.719183,6.015326),0.000000);
	 	 increment=rate;
	 	 ds[2]+=increment;
	}
	 memory=step-0;
	 if(memory>=0){
	 	 rate=MAX(3.449273,0.384984)*HillR(history[2][memory][ncell],0.044938,3.266732);
	 	 increment=rate;
	 	 ds[3]+=increment;
	}
